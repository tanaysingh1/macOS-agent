...
class MacUITreeBuilder:
    """Builds a recursive UI tree structure using macOS Accessibility API"""
    
    def __init__(self):
        self.highlight_index = 0
        self._element_cache: Dict[int, MacElementNode] = {}
    ...
    async def _process_element(self, element: 'AXUIElement', pid: int, parent: Optional[MacElementNode] = None, depth: int = 0) -> Optional[MacElementNode]:
        """Process a single UI element recursively"""
        ...
        try:
            ...
            if node.is_interactive:
                ...
                logger.debug(f'Added interactive element {role} with index {node.highlight_index}')
    ...
    async def build_tree(self, pid: Optional[int] = None, notification_callback=None) -> Optional[MacElementNode]:
    ...
    def get_element_by_index(self, index: int) -> Optional[MacElementNode]:
        """Get an element by its highlight index (for action execution)"""
        return self._element_cache.get(index)

    def get_all_interactive_elements(self) -> Dict[int, MacElementNode]:
        """Get all interactive elements with their indices"""
        return self._element_cache.copy()

    def get_stats(self) -> Dict[str, Any]:
        """Get statistics about the current tree"""
        return {
            'total_interactive_elements': len(self._element_cache),
            'processed_elements_count': len(self._processed_elements),
            'next_highlight_index': self.highlight_index,
            'current_app_pid': self._current_app_pid
        }